-- Real restaurant QR codes are bearer capabilities. The intentionally public
-- demo restaurant remains directly reachable at /t/12 for portfolio review.
alter table public.restaurants
add column is_demo boolean not null default false;

drop function public.create_customer_order(text, text, text, text, jsonb);
...
begin
  ...
  select id, is_demo into v_restaurant_id, v_restaurant_is_demo
  ...
  where restaurant_id = v_restaurant_id
    and table_number = btrim(p_table_number)
    and active
    and (v_restaurant_is_demo or qr_token = p_qr_token);
  if v_table_id is null then
    raise exception using errcode = '22023', message = 'Meja atau kode QR tidak valid.';
  end if;
  ...
  loop
    ...
    if jsonb_typeof(v_selected_options) <> 'array'
      or jsonb_array_length(v_selected_options) > 20 then
...
-- Serialize the rolling-window check for each customer session so concurrent
-- requests cannot all pass the same pre-insert count.
